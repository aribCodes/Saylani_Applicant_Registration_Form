...
function Registration() {
  ...
  const { coursesData,jwtToken } = useGlobalContext({}) || {};

  useEffect(() => {
    if (coursesData.length > 0) {
      console.log("courses available: ", coursesData);
    }
  }, [coursesData]);
  ...
  const onFinish = async (values) => {
    console.log("CNIC Value",values.cnic)
                    ...
                    >
                      {coursesData.length > 0 ? (
                        coursesData.map((element,index) => {
                          return (
                            <Select.Option key={index} value={element.courseName}>
                              {element.courseName}
                        ...
                        <Select.Option value="No Courses">
